...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-lg bg-[#15171b] shadow-[0_18px_50px_rgba(0,0,0,0.18)] transition hover:-translate-y-1 ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[#30303c] text-white">
        <section className="mx-auto max-w-[1160px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-lg bg-[#15171b] p-6 shadow-[0_30px_90px_rgba(0,0,0,0.18)] md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div>
              <p className="text-xs font-black uppercase text-[#00d084]">{pagesContent.search.hero.badge}</p>
            ...
            <form action="/search" className="self-end rounded-lg border border-white/10 bg-[#30303c] p-4 sm:p-5">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-md bg-[#00d084] px-6 text-sm font-black uppercase text-[#07110d] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/listings" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-black px-5 py-3 text-sm font-black">Browse latest <ArrowRight className="h-4 w-4" /></Link>
          </div>
